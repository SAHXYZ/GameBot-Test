...
def check_cooldown(user: dict, cmd: str, cooldown_seconds: int):
    ...
    Returns:
        ok (bool): True if user can use command
        remaining (int): seconds left
        pretty (str): human-readable remaining
    """
    user = ensure_cd(user)
    now = int(time.time())
...
def update_cooldown(user: dict, cmd: str):
    ...
    NOTE: Remember to save changes using `db.update_user(uid, user)`
    """
    user = ensure_cd(user)
